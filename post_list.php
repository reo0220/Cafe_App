<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['search'])){
        ...
        //全部入力されているまたは選択されている場合か、店名のみ入力されていない場合
        if((!empty($name_search) && $place_search != "0" && $price_search != "0") || (empty($name_search) && $place_search != "0" && $price_search != "0")){
            ...
            $counts = $dbh_search->query($sql_counts);
            $count = $counts->fetch();
        }elseif(!empty($name_search) && $place_search != "0" && $price_search === "0"){//店名と場所のみ、選択入力されている場合か、場所のみ選択している場合
            $sql_search = "SELECT
                                posts.post_id,
                                posts.user_id,
                                posts.name AS posts_name,
            ...
            $counts = $dbh_search->query($sql_counts);
            $count = $counts->fetch();
        }elseif(!empty($name_search) && $place_search === "0" && $price_search != "0"){//店名と価格帯のみ選択入力されている場合か、価格帯のみ選択している場合
            $sql_search = "SELECT
                                posts.post_id,
                                posts.user_id,
                                posts.name AS posts_name,
    ...
    <body>
        ...
        <main class = "main1">
            <div class="main2">
                ...
                <?php if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['search']) && $count['cnt'] === "0"):?>
                    <h1>該当の投稿がありません。</h1>
                ...
                <?php elseif($count === "" || empty($_POST['search'])):?>                        
